...
class MatrixFactorization_BPR_Theano(object):
    ...
    def _generate_train_model_function(self):
        ...
        obj = T.sum(
            T.log(T.nnet.sigmoid(x_uij))
            - self.lambda_u * (self.W[u] ** 2).sum(axis=1)
            - self.lambda_i * (self.H[i] ** 2).sum(axis=1)
            - self.lambda_j * (self.H[j] ** 2).sum(axis=1)
        ...
        updates = [
            (self.W, self.W - self.learning_rate * g_cost_W),
    ...
    def top_predictions(self, user_index, topn=10):
        ...
        return [
            item_index
            for item_index in numpy.argsort(self.predictions(user_index))
