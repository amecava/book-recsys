...
from Base.Evaluation.metrics import (
    roc_auc,
    precision,
    ...
    for cutoff in results_run.keys():
        ...
        for metric in results_run_current_cutoff.keys():
            output_str += "{}: {:.{n_decimals}f}, ".format(
                metric, results_run_current_cutoff[metric], n_decimals=n_decimals
            )
    ...
    def _compute_metrics_on_recommendation_list(
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[
                    EvaluatorMetrics.COVERAGE_ITEM_CORRECT.value
                ].add_recommendations(
                ...
                results_current_cutoff[
                    EvaluatorMetrics.COVERAGE_USER_CORRECT.value
        ...
        if time.time() - self._start_time_print > 30 or self._n_users_evaluated == len(
...
class EvaluatorNegativeItemSample(Evaluator):
    ...
    def __init__(
        ...
        self.URM_items_to_rank = sps.csr_matrix(
            self.URM_test.copy().astype(np.bool)
