...
class MyTestCase(unittest.TestCase):
    def test_Gini_Index(self):
        ...
        assert np.isclose(
            1.0, gini_index.get_metric_value(), atol=1e-2
        ), "Gini_Index metric incorrect"

        gini_index.recommended_counter = np.ones(n_items) * 1e-12
        gini_index.recommended_counter[0] = 1.0
        assert np.isclose(
            0.0, gini_index.get_metric_value(), atol=1e-2
        ), "Gini_Index metric incorrect"
    ...
    def test_Shannon_Entropy(self):
        ...
        assert np.isclose(
            9.96, shannon_entropy.get_metric_value(), atol=1e-2
        ), "metric incorrect"

        shannon_entropy.recommended_counter = np.zeros(n_items)
        shannon_entropy.recommended_counter[0] = 1.0
        assert np.isclose(
            0.0, shannon_entropy.get_metric_value(), atol=1e-3
        ), "metric incorrect"

        shannon_entropy.recommended_counter = np.random.uniform(0, 100, n_items).astype(
            np.int
        )
        assert np.isclose(
            9.6, shannon_entropy.get_metric_value(), atol=1e-1
        ), "metric incorrect"
    ...
    def test_Diversity_list_all_equals(self):
        ...
        assert np.isclose(
            diversity_cooccurrence, object_diversity, atol=1e-4
        ), "metric incorrect"
    ...
    def test_AUC(self):
        ...
        self.assertTrue(np.allclose(roc_auc(is_relevant), (2.0 / 3 + 1.0 / 3) / 2))
    ...
    def test_Recall(self):
        ...
        self.assertTrue(np.allclose(recall(is_relevant, pos_items), 3.0 / 4))
    ...
    def test_Precision(self):
        ...
        is_relevant = np.in1d(ranked_list_1, pos_items, assume_unique=True)
        self.assertTrue(np.allclose(precision(is_relevant), 3.0 / 5))

        is_relevant = np.in1d(ranked_list_2, pos_items, assume_unique=True)
        self.assertTrue(np.allclose(precision(is_relevant), 4.0 / 5))
    ...
    def test_RR(self):
        ...
        is_relevant = np.in1d(ranked_list_1, pos_items, assume_unique=True)
        self.assertTrue(np.allclose(rr(is_relevant), 1.0 / 2))

        is_relevant = np.in1d(ranked_list_2, pos_items, assume_unique=True)
        self.assertTrue(np.allclose(rr(is_relevant), 1.0))
    ...
    def test_MAP(self):
        ...
        is_relevant = np.in1d(ranked_list_1, pos_items, assume_unique=True)
        self.assertTrue(
            np.allclose(map(is_relevant, pos_items), (1.0 / 2 + 2.0 / 4 + 3.0 / 5) / 4)
        )
        ...
        self.assertTrue(
            np.allclose(
                map(is_relevant, pos_items), (1.0 + 2.0 / 7 + 3.0 / 8 + 4.0 / 9) / 4
            )
        )
