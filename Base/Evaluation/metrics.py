...
def precision_recall_min_denominator(is_relevant, n_test_items):
    ...
    if len(is_relevant) == 0:
        ...
        precision_score = np.sum(is_relevant, dtype=np.float32) / min(
...
class RMSE(_Metrics_Object):
    ...
    def add_recommendations(
        self, all_items_predicted_ratings, relevant_items, relevant_items_rating
    ):

        assert len(relevant_items) == len(
            relevant_items_rating
        ), "RMSE: the list of relevant items and of the corresponding rating do not have the same length"

        assert np.all(relevant_items_rating >= self._min_rating) and np.all(
            relevant_items_rating <= self._max_rating
        ...
        all_items_clipped_ratings = np.clip(
            all_items_predicted_ratings, a_min=self._min_rating, a_max=self._max_rating
        )

        relevant_items_error = (
            all_items_clipped_ratings[relevant_items] - relevant_items_rating
        ) ** 2
    ...
    def merge_with_other(self, other_metric_object):
        assert (
            other_metric_object is RMSE
        ), "RMSE: attempting to merge with a metric object of different type"
...
class Coverage_Test_Correct(_Global_Item_Distribution_Counter):
    ...
    def add_recommendations(self, recommended_items_ids, is_relevant):
        super(Coverage_Test_Correct, self).add_recommendations(
...
class TestAUC(unittest.TestCase):
    def runTest(self):
        ...
        self.assertTrue(
            np.allclose(roc_auc(ranked_list, pos_items), (2.0 / 3 + 1.0 / 3) / 2)
        )
