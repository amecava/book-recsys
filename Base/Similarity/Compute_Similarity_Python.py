...
class Compute_Similarity_Python:
    ...
    def applyAdjustedCosine(self):
        ...
        rowAverage[nonzeroRows] = (
            sumPerRow[nonzeroRows] / interactionsPerRow[nonzeroRows]
        )
        ...
        while end_row < self.n_rows:
            ...
            self.dataMatrix.data[
                ...
                rowAverage[start_row:end_row], interactionsPerRow[start_row:end_row]
            )
    ...
    def applyPearsonCorrelation(self):
        ...
        colAverage[nonzeroCols] = (
            sumPerCol[nonzeroCols] / interactionsPerCol[nonzeroCols]
        )
        ...
        while end_col < self.n_columns:
            ...
            self.dataMatrix.data[
                ...
                colAverage[start_col:end_col], interactionsPerCol[start_col:end_col]
            )
    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size=100):
        ...
        if (
            end_col is not None
            and end_col > start_col_local
            and end_col < self.n_columns
        ):
        ...
        while start_col_block < end_col_local:
            ...
            for col_index_in_block in range(this_block_size):
                ...
                if self.normalize:
                    ...
                    if self.asymmetric_cosine:
                        denominator = (
                            sumOfSquared_to_alpha[columnIndex]
                            * sumOfSquared_to_1_minus_alpha
                            + self.shrink
                            + 1e-6
                        )
                    else:
                        denominator = (
                            sumOfSquared[columnIndex] * sumOfSquared
                            + self.shrink
                            + 1e-6
                        )
                ...
                # Apply the specific denominator for Tanimoto
                    ...
                    denominator = (
                        sumOfSquared[columnIndex] + sumOfSquared + self.shrink + 1e-6
                    )
                    this_column_weights = np.multiply(
                        this_column_weights, 1 / denominator
                    )
                    ...
                    denominator = (
                        this_column_weights
                        + (sumOfSquared[columnIndex] - this_column_weights)
                        * self.tversky_alpha
                        + (sumOfSquared - this_column_weights) * self.tversky_beta
                        + self.shrink
                        + 1e-6
                    )
                    this_column_weights = np.multiply(
            ...
            if (
                time.time() - start_time_print_batch >= 30
                or end_col_block == end_col_local
            ):
                columnPerSec = processedItems / (time.time() - start_time + 1e-9)

                print(
                    "Similarity column {} ( {:2.0f} % ), {:.2f} column/sec, elapsed time {:.2f} min".format(
                        processedItems,
                        processedItems / (end_col_local - start_col_local) * 100,
                        columnPerSec,
