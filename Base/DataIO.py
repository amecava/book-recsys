...
def json_not_serializable_handler(o):
    ...
    raise TypeError(
        "json_not_serializable_handler: object '{}' is not serializable.".format(
            type(o)
        )
    )
...
class DataIO(object):
    ...
    def _get_temp_folder(self, file_name):
        ...
        current_temp_folder = "{}{}_{}/".format(
            self.folder_path, self._DEFAULT_TEMP_FOLDER, file_name
        )
    ...
    def save_data(self, file_name, data_dict_to_save):
        ...
        # Save list objects
        attribute_to_json_file[
            ".DataIO_attribute_to_file_name"
        ] = attribute_to_file_name.copy()
        ...
        with zipfile.ZipFile(
            self.folder_path + file_name, "w", compression=zipfile.ZIP_DEFLATED
        ) as myzip:
