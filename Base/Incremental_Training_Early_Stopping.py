...
class Incremental_Training_Early_Stopping(object):
    ...
    def _train_with_early_stopping(
        ...
        assert (
            epochs_max >= 0
        ), "{}: Number of epochs_max must be >= 0, passed was {}".format(
            algorithm_name, epochs_max
        )
        assert (
            epochs_min >= 0
        ), "{}: Number of epochs_min must be >= 0, passed was {}".format(
        ...
        while epochs_current < epochs_max and not convergence:
            ...
            # Determine whether a validaton step is required
                ...
                if not np.isfinite(current_metric_value):
                    ...
                    assert (
                        False
                    ), "{}: metric value is not a finite number, terminating!".format(
                        self.RECOMMENDER_NAME
                    )

                if (
                    self.best_validation_metric is None
                    or self.best_validation_metric < current_metric_value
                ):
