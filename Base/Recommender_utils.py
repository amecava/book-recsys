...
def similarityMatrixTopK(item_weights, k=100, verbose=False):
    ...
    assert (
        item_weights.shape[0] == item_weights.shape[1]
    ...
    W_sparse = sps.csc_matrix(
        (data, rows_indices, cols_indptr), shape=(nitems, nitems), dtype=np.float32
    )

    if verbose:
        print(
            "Sparse TopK matrix generated in {:.2f} seconds".format(
...
def removeTopPop(URM_1, URM_2=None, percentageToRemove=0.2):
    ...
    item_pop = np.asarray(
        item_pop
    ).squeeze()  # necessary to convert it into a numpy.array of size (nitems,)
    ...
    itemMask = np.in1d(
        np.arange(len(popularItemsSorted)),
        popularItemsSorted[:numItemsToRemove],
        invert=True,
    )
...
def get_unique_temp_folder(input_temp_folder_path):
    ...
    while os.path.isdir(progressive_temp_folder_name):
        ...
        progressive_temp_folder_name = (
            input_temp_folder_path + "_" + str(counter_suffix)
        )
