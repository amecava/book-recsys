...
def get_result_string_evaluate_on_validation(results_run_single_cutoff, n_decimals=7):
    ...
    for metric in results_run_single_cutoff.keys():
        output_str += "{}: {:.{n_decimals}f}, ".format(
            metric, results_run_single_cutoff[metric], n_decimals=n_decimals
        )
...
class SearchAbstractClass(object):
    ...
    def _set_search_attributes(
        ...
        if save_model not in self._SAVE_MODEL_VALUES:
            raise ValueError(
                "{}: parameter save_model must be in '{}', provided was '{}'.".format(
    ...
    def _fit_model(self, current_fit_parameters):
        ...
        self._print(
            "{}: Testing config: {}".format(self.ALGORITHM_NAME, current_fit_parameters)
        )
        ...
        if print_log:
            self._write_log(
                "{}: Best config evaluated with evaluator_test. Config: {} - results:\n{}\n".format(
                    self.ALGORITHM_NAME, current_fit_parameters_dict, result_string
                )
            )
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        if self.resume_from_saved:
            result_on_last_saved_flag = (
                self.metadata_dict["result_on_last"] is not None
                and self.metadata_dict["time_on_last_train"] is not None
                and self.metadata_dict["time_on_last_test"] is not None
            )
    ...
    def _objective_function(self, current_fit_parameters_dict):
        ...
        try:
            ...
            self.metadata_dict["hyperparameters_list"][
                self.model_counter
            ] = current_fit_parameters_dict.copy()
            ...
            (
                ...
                train_time,
                evaluation_time,
            ) = self._evaluate_on_validation(current_fit_parameters_dict)
            ...
            if isinstance(recommender_instance, Incremental_Training_Early_Stopping):
                ...
                n_epochs_early_stopping_dict = (
                    recommender_instance.get_early_stopping_final_epochs_dict()
                )
            ...
            if self.metadata_dict["result_on_validation_best"] is None:
                ...
                new_best_config_found = (
                    best_solution_val < result_dict[self.metric_to_optimize]
                )
            ...
            if new_best_config_found:
                ...
                if (
                    self.evaluator_test is not None
                    and self.evaluate_on_test_each_best_solution
                ):
            ...
            if new_best_config_found:
                ...
                if (
                    ...
                    self.metadata_dict["result_on_test_list"][
                        self.model_counter
                    ] = result_dict_test.copy()
                    self.metadata_dict["time_on_test_list"][
                        self.model_counter
                    ] = evaluation_test_time

                    (
                        self.metadata_dict["time_on_test_total"],
