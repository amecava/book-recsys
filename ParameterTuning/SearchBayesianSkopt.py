...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def _resume_from_saved(self):
        ...
        result_on_validation_list_saved = self.metadata_dict[
            "result_on_validation_list"
        ]
        ...
        while (
            self.model_counter < len(hyperparameters_list_saved)
            and hyperparameters_list_saved[self.model_counter] is not None
        ):

            hyperparameters_config_saved = hyperparameters_list_saved[
                self.model_counter
            ]
            ...
            for index in range(len(self.hyperparams_names)):
                ...
                if (
                    isinstance(self.hyperparams_values[index], Categorical)
                    and self.hyperparams_values[index].transformed_size == 1
                ):
            ...
            if validation_result is None:
                ...
                assert (
                    self.metadata_dict["exception_list"][self.model_counter] is not None
                ...
                result_on_validation_list_input.append(
                    -validation_result[self.metric_to_optimize]
                )
