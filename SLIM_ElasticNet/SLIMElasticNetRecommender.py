    ...
    def fit(self, l1_ratio=0.1, alpha=1.0, positive_only=True, topK=100):
        ...
        for currentItem in range(n_items):
            ...
            if time.time() - start_time_printBatch > 300 or currentItem == n_items - 1:
                self._print(
                    "Processed {} ( {:.2f}% ) in {:.2f} {}. Items per second: {:.2f}".format(
                        currentItem + 1,
...
class MultiThreadSLIM_ElasticNet(
    SLIMElasticNetRecommender, BaseItemSimilarityMatrixRecommender
):
    def __init__(self, URM_train, verbose=True):
    ...
    def fit(
        ...
        assert (
            l1_ratio >= 0 and l1_ratio <= 1
        ), "SLIM_ElasticNet: l1_ratio must be between 0 and 1, provided value was {}".format(
            l1_ratio
        )
