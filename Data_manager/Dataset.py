...
class Dataset(object):
    ...
    def save_data(self, save_folder_path):
        ...
        if self._HAS_ICM:
            dataIO.save_data(
                data_dict_to_save=self.AVAILABLE_ICM, file_name="dataset_ICM"
            )

            dataIO.save_data(
                data_dict_to_save=self.AVAILABLE_ICM_feature_mapper,
                file_name="dataset_ICM_mappers",
            )

        if self._HAS_UCM:
            dataIO.save_data(
                data_dict_to_save=self.AVAILABLE_UCM, file_name="dataset_UCM"
            )

            dataIO.save_data(
                data_dict_to_save=self.AVAILABLE_UCM_feature_mapper,
                file_name="dataset_UCM_mappers",
            )

        if self._HAS_additional_mapper:
            dataIO.save_data(
                data_dict_to_save=self.additional_data_mapper,
    ...
    def print_statistics(self):
        ...
        if self._HAS_ICM:
            ...
            for ICM_name, ICM_object in self.AVAILABLE_ICM.items():
                ...
                format_string = (
                    "2E"
                    if np.max([np.abs(min_value), np.abs(max_value)]) > 100
                    else "2f"
                )
    ...
    def verify_data_consistency(self):
        ...
        assert n_interactions != 0, (
            print_preamble + "Number of interactions in URM is 0"
        )
