...
class DatasetMapperManager(object):
    ...
    def add_URM(self, URM_dataframe: pd.DataFrame, URM_name):
        ...
        assert set(["UserID", "ItemID", "Data"]).issubset(
            set(URM_dataframe.columns)
        ), "Dataframe columns not correct"
        assert all(
            is_string_dtype(URM_dataframe[ID_column])
            for ID_column in ["UserID", "ItemID"]
    ...
    def add_ICM(self, ICM_dataframe: pd.DataFrame, ICM_name):
        ...
        assert all(
            is_string_dtype(ICM_dataframe[ID_column])
            for ID_column in ["ItemID", "FeatureID"]
        ), "ID columns must be strings"
        assert (
            not self.__Dataset_finalized
        ), "Dataset mappers have already been generated, adding new data is forbidden"
    ...
    def add_UCM(self, UCM_dataframe: pd.DataFrame, UCM_name):
        ...
        assert all(
            is_string_dtype(UCM_dataframe[ID_column])
            for ID_column in ["UserID", "FeatureID"]
        ), "ID columns must be strings"
        assert (
            not self.__Dataset_finalized
        ), "Dataset mappers have already been generated, adding new data is forbidden"
