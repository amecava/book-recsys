...
class DataSplitter_leave_k_out(_DataSplitter):
    ...
    def __init__(
        ...
        assert (
            k_out_value >= 1
        ), "{}: k_out_value must be  greater or equal than 1".format(
    ...
    def get_statistics_URM(self):
        ...
        if self.use_validation_set:
            statistics_string += (
                "\tValidation \tinteractions {}, \tdensity {:.2E}\n".format(
    ...
    def _split_data_from_original_dataset(self, save_folder_path):
        ...
        if not self.allow_cold_users:
            ...
            self.SPLIT_GLOBAL_MAPPER_DICT[
                ...
                np.arange(0, len(self.removed_cold_users), dtype=np.int)[
                    self.removed_cold_users
                ],
            )
    ...
    def _save_split(self, save_folder_path):
        ...
        if save_folder_path:
            ...
            dataIO.save_data(
                data_dict_to_save=split_parameters_dict,
                file_name="split_parameters" + name_suffix,
            )

            dataIO.save_data(
                data_dict_to_save=self.SPLIT_GLOBAL_MAPPER_DICT,
                file_name="split_mappers" + name_suffix,
            )

            dataIO.save_data(
                data_dict_to_save=self.SPLIT_URM_DICT,
                file_name="split_URM" + name_suffix,
            )

            if len(self.SPLIT_ICM_DICT) > 0:
                dataIO.save_data(
                    data_dict_to_save=self.SPLIT_ICM_DICT,
                    file_name="split_ICM" + name_suffix,
                )

                dataIO.save_data(
                    data_dict_to_save=self.SPLIT_ICM_MAPPER_DICT,
                    file_name="split_ICM_mappers" + name_suffix,
                )

            if len(self.SPLIT_UCM_DICT) > 0:
                dataIO.save_data(
                    data_dict_to_save=self.SPLIT_UCM_DICT,
                    file_name="split_UCM" + name_suffix,
                )

                dataIO.save_data(
                    data_dict_to_save=self.SPLIT_UCM_MAPPER_DICT,
    ...
    def _verify_data_consistency(self):
        ...
        assert all(
            URM_name in URM_to_load_list for URM_name in self.SPLIT_URM_DICT.keys()
        ...
        assert self.SPLIT_URM_DICT["URM_train"].nnz != 0, (
            print_preamble + "Number of interactions in URM Train is 0"
        )
        assert self.SPLIT_URM_DICT["URM_test"].nnz != 0, (
            print_preamble + "Number of interactions in URM Test is 0"
        )
