...
def split_big_CSR_in_columns(sparse_matrix_to_split, num_split=2):
    ...
    assert (
        sparse_matrix_to_split.shape[1] > 0
    ), "split_big_CSR_in_columns: sparse_matrix_to_split has no columns"
    assert (
    ...
    for num_current_split in range(num_split):
        ...
        print(
            "split_big_CSR_in_columns: Split {}, columns: {}-{}".format(
...
def remove_Dataframe_duplicates(
    ...
    n_unique_data_points = dataframe.drop_duplicates(
        unique_values_in_columns, keep="first", inplace=False
    ).shape[0]

    assert (
        n_unique_data_points == n_data_points
    ), "remove_Dataframe_duplicates: duplicate values found"
...
def load_CSV_into_Dataframe(
    ...
    num_unique_user_item_ids = df_original.drop_duplicates(
        ["UserID", "ItemID"], keep="first", inplace=False
    ).shape[0]
...
def remove_features(
    ICM, min_occurrence=5, max_percentage_occurrence=0.30, reconcile_mapper=None
):
    """
...
def reconcile_mapper_with_removed_tokens(key_to_value_dict, values_to_remove):
    ...
    mapper_values_array = (
        np.ones(max(key_to_value_dict.values()) + 1, dtype=np.int) * -np.inf
    )
    ...
    for value_to_remove in values_to_remove:
        ...
        assert (
            value_to_remove in value_to_key
        ), "Value to be removed from dictionary is not in dictionary"
...
def invert_dictionary(id_to_index):
    ...
    for id in id_to_index.keys():
        ...
        assert (
            index not in index_to_id
        ), "Dictionary is not invertible as it contains duplicate values."
