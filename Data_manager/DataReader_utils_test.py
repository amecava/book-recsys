...
class MyTestCase(unittest.TestCase):
    def test_reconcile_mapper_with_removed_tokens(self):
        ...
        reconciled_mapper = reconcile_mapper_with_removed_tokens(
            original_mapper.copy(), [0]
        )
        assert reconciled_mapper == {
            "b": 0,
            "c": 1,
            "d": 2,
            "e": 3,
        }, "reconciled_mapper not matching control"

        reconciled_mapper = reconcile_mapper_with_removed_tokens(
            original_mapper.copy(), [4]
        )
        assert reconciled_mapper == {
            "a": 0,
            "b": 1,
        ...
        reconciled_mapper = reconcile_mapper_with_removed_tokens(
            original_mapper.copy(), [0, 1, 2, 3, 4]
        )
    ...
    def test_split_big_CSR_in_columns(self):
        ...
        assert np.allclose(
            sparse_matrix.toarray(), split_rebuilt.toarray()
        ), "split_rebuilt not matching sparse_matrix"
