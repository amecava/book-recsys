...
class IncrementalSparseMatrix_ListBased(object):
    ...
    def add_data_lists(self, row_list_to_add, col_list_to_add, data_list_to_add):
        ...
        assert len(row_list_to_add) == len(col_list_to_add) and len(
            row_list_to_add
        ) == len(
            data_list_to_add
        ...
        col_list_index = [
            self._get_column_index(column_id) for column_id in col_list_to_add
        ]
    ...
    def get_SparseMatrix(self):
        ...
        sparseMatrix = sps.csr_matrix(
            (self._data_list, (self._row_list, self._col_list)), shape=shape
        )
        ...
        for data_point_index in range(len(row_list_to_add)):
            ...
            if self._next_cell_pointer == len(self._row_array):
                self._row_array = np.concatenate(
                    (
                        self._row_array,
                        np.zeros(self._dataBlock, dtype=self._dtype_coordinates),
                    )
                )
                ...
                self._data_array = np.concatenate(
                    (
                        self._data_array,
                        np.zeros(self._dataBlock, dtype=self._dtype_data),
                    )
                )
            ...
            self._data_array[self._next_cell_pointer] = data_list_to_add[
                data_point_index
            ]
