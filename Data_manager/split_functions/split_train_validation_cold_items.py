...
def _select_train_warm_items(
    URM_all, train_item_percentage, train_interaction_percentage=None
):
    """
    ...
    while not terminate:
        ...
        if train_interaction_percentage is not None:
            ...
            if (
                current_train_interaction_percentage
                < train_interaction_percentage * 0.9
            ):
                ...
                current_train_interaction_percentage
                > train_interaction_percentage * 1.1
            ):
    ...
    assert (
        sample_successful
    ), "Unable to select the train items with the desired specifications"
...
def _zero_out_values(sparse_matrix, columns_to_zero=None, rows_to_zero=None):
    ...
    if columns_to_zero is not None:
        sparse_matrix = _zero_out_values(
            sparse_matrix.T, rows_to_zero=columns_to_zero
        ).T
...
def split_train_in_two_cold_items(
    URM_all, ICM_list=None, train_item_percentage=0.1, train_interaction_percentage=None
):
    """
    ...
    ), "train_item_percentage must be a value between 0.0 and 1.0, provided was '{}'".format(
