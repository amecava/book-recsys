...
class SLIM_BPR(BaseRecommender):
    ...
    def __init__(
        self, URM_train, lambda_i=0.0025, lambda_j=0.00025, learning_rate=0.05
    ):
    ...
    def updateFactors(self, user_id, pos_item_id, neg_item_id):
        ...
        for userSeenItem in userSeenItems:
            prediction += (
                self.S[pos_item_id, userSeenItem] - self.S[neg_item_id, userSeenItem]
            )
        ...
        for userSeenItem in userSeenItems:
            ...
            if pos_item_id != userSeenItem:
                update = (
                    logisticFunction - self.lambda_i * self.S[pos_item_id, userSeenItem]
                )
            ...
            if neg_item_id != userSeenItem:
                update = (
                    -logisticFunction
                    - self.lambda_j * self.S[neg_item_id, userSeenItem]
                )
    ...
    def fit(self, epochs=15):
        ...
        for currentEpoch in range(epochs):
            ...
            self.epochIteration()
            print(
                "Epoch {} of {} complete in {:.2f} minutes".format(
                    currentEpoch + 1, epochs, float(time.time() - start_time_epoch) / 60
                )
            )

        print(
            "Train completed in {:.2f} minutes".format(
    ...
    def epochIteration(self):
        ...
        for numSample in range(numPositiveIteractions):
            ...
            if numSample % 5000 == 0:
                print(
                    "Processed {} ( {:.2f}% ) in {:.4f} seconds".format(
                        numSample,
