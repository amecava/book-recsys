...
class SLIM_BPR_Cython(
    ...
    def __init__(
        self, URM_train, verbose=True, free_mem_threshold=0.5, recompile_cython=False
    ):

        super(SLIM_BPR_Cython, self).__init__(URM_train, verbose=verbose)

        assert (
            free_mem_threshold >= 0.0 and free_mem_threshold <= 1.0
        ), "SLIM_BPR_Recommender: free_mem_threshold must be between 0.0 and 1.0, provided was '{}'".format(
            free_mem_threshold
        )
    ...
    def fit(
        ...
        **earlystopping_kwargs
        ...
        if self.positive_threshold_BPR is not None:
            ...
            ), "SLIM_BPR_Cython: URM_train_positive is empty, positive threshold is too high"
