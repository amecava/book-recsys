...
class CFW_D_Similarity_Linalg(BaseItemSimilarityMatrixRecommender):
    ...
    def __init__(self, URM_train, ICM, S_matrix_target):
        ...
        if URM_train.shape[1] != ICM.shape[0]:
            raise ValueError(
                "Number of items not consistent. URM contains {} but ICM contains {}".format(
                    URM_train.shape[1], ICM.shape[0]
                )
            )

        if S_matrix_target.shape[0] != S_matrix_target.shape[1]:
            raise ValueError(
                "Items imilarity matrix is not square: rows are {}, columns are {}".format(
                    S_matrix_target.shape[0], S_matrix_target.shape[1]
                )
            )

        if S_matrix_target.shape[0] != ICM.shape[0]:
            raise ValueError(
                "Number of items not consistent. S_matrix contains {} but ICM contains {}".format(
                    S_matrix_target.shape[0], ICM.shape[0]
                )
            )
    ...
    def _generateTrainData_low_ram(self):
        ...
        for row_index in range(self.n_items):
            ...
            for index in range(len(is_common)):
                ...
                if is_common[index]:
                    ...
                    if self.normalize_similarity:
                        new_data_value *= (
                            sum_of_squared_features[row_index]
            ...
            if (
                time.time() - start_time_batch > 30
                or num_samples == S_matrix_contentKNN.nnz * (1 + self.add_zeros_quota)
            ):
        ...
        self._writeLog(
            self.RECOMMENDER_NAME
            + ": Content S structure has {} out of {} ( {:.2f}%) nonzero collaborative cells".format(
